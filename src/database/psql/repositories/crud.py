...
from src.database.psql.interfaces.crud import AbstractCRUDRepository
from src.database.psql.models.base import Base
...
class CRUDRepository[M: Base](AbstractCRUDRepository[M]):
    ...
    async def insert(self, **values: Any) -> M | None:
    ...
    async def update_many(self, data: Sequence[Mapping[str, Any]]) -> Result[Any]:
    ...
    async def upsert(self, *conflict_columns: str, **values: Any) -> M | None:
        stmt = (
